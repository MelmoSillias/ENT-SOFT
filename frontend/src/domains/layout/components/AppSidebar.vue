...
const activeKey = computed(() => {
  ...
  for (const item of props.menuModel) {
    if (item.routeName === routeName || item.activeRouteNames?.includes(routeName)) {
      return item.key
    }

    if (item.items) {
      for (const child of item.items) {
        if (child.routeName === routeName || child.activeRouteNames?.includes(routeName)) {
          return child.key
        }
      }
    }
  ...
  <div class="app-sidebar-host">
    <div v-if="sidebarMode === 'fixed'" class="app-sidebar-wrapper">
      ...
      <div class="app-sidebar__container">
        <!-- Logo Section -->
        <div v-if="logoAtTop" class="app-sidebar__logo app-sidebar__logo--stacked">
          <div
        ...
        <!-- Logo Center -->
        <div v-if="!logoAtTop" class="app-sidebar__logo app-sidebar__logo--center app-sidebar__logo--stacked">
          <div
            class="app-sidebar__logo-mark"
            :class="{ 'app-sidebar__logo-mark--brand': brand.logoUrl }"
          >
          ...
          <div v-if="!collapsed" class="app-sidebar__logo-text">
            <h2 class="app-sidebar__brand">{{ brand.name }}</h2>
            <p v-if="brand.tagline" class="app-sidebar__tagline">{{ brand.tagline }}</p>
          </div>
        </div>
      </div>
      </aside>
    ...
    >
      <template #header>
        <div class="app-sidebar__logo app-sidebar__logo--stacked">
          <div
            class="app-sidebar__logo-mark"
            :class="{ 'app-sidebar__logo-mark--brand': brand.logoUrl }"
          >
