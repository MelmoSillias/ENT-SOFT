...
const groupDisplayLabel = computed(() => props.group?.shortLabel || props.group?.label)

const groupLabelTitle = computed(() => {
  if (props.group?.shortLabel && props.group.shortLabel !== props.group.label) {
    return props.group.label
  }
  return undefined
})

const itemDisplayLabel = (item) => item.shortLabel || item.label

const itemLabelTitle = (item) => {
    ...
    >
      <div ref="contentRef" class="app-secondary-sidebar__content">
        <div class="app-secondary-sidebar__header">
          ...
          <h3 class="app-secondary-sidebar__title" :title="groupLabelTitle">{{ groupDisplayLabel }}</h3>
        </div>
        <div class="app-secondary-sidebar__items">
          <button
            ...
            :title="itemLabelTitle(item)"
            :aria-label="item.label"
            @click="handleItemClick(item)"
          >
            <i v-if="item.icon" :class="['app-secondary-sidebar__item-icon', item.icon]"></i>
            <span class="app-secondary-sidebar__item-label">{{ itemDisplayLabel(item) }}</span>
          </button>
        </div>
      </div>
    </aside>
