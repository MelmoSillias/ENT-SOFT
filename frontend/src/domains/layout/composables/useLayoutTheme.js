...
const resolveAccentHex = (accentName, accentColor, isDark = false) => {
  if (accentName === 'custom') {
    return normalizeHex(accentColor) || accentColorMap.emerald
  }

  if (isDark && accentName === 'cargo') {
    return accentColorMap.cargoDark
  }
...
export function useLayoutTheme() {
  ...
  watch(
    ...
    () => {
      ...
      const resolvedAccent = resolveAccentHex(accentName.value, accentColor.value, shouldApplyDarkMode.value)

      if (accentName.value === 'custom') {
        root.style.setProperty('--layout-accent', resolvedAccent)
        root.style.setProperty(
          '--layout-accent-strong',
          shouldApplyDarkMode.value ? mixHex(resolvedAccent, '#ffffff', 0.35) : mixHex(resolvedAccent, '#000000', 0.28)
        )
        root.style.setProperty('--layout-accent-soft', toRgba(resolvedAccent, shouldApplyDarkMode.value ? 0.22 : 0.16))
      } else {
